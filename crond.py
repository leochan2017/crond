...
def sendMail():
    ...
    mail_receivers = ['xxxxxx@qq.com'] # 接收邮箱
    message = 'Your Process is Dead at ' + time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
    ...
    try:
        ...
        if state[0] == 235:
            smtpObj.sendmail(mail_sender, mail_receivers, message)
            print '邮件发送成功'
        smtpObj.quit()
